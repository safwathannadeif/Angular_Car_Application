...
export class CustomerComponent implements OnInit {
  ...
  constructor(
    ...
    private comService: Comservices,
  ...
  saveForm(formvalue, isValid: boolean) {
    ...
    if (n === 1) {
      this.getByCusIdHttp(cus);
    }
    if (n === 2) {
      this.getByCusNameHttp(cus);
    }
  }
  ngOnInit(): void {
    this.initGrpCtl();
  ...
  SearchValidator: ValidatorFn = (formGroup: FormGroup) => {
    ...
    if (selCtlValue.valueN != this.curNValue) {
      // chg in option Happend
    ...
    // tslint:disable-next-line: quotemark
    console.log('searchValidatorFn cusCtlValue=' + cusCtlValue); // tslint:disable-next-line: curly
  ...
  getByCusNameHttp(cusName: string) {
    this.comService.getCusByNames(cusName).subscribe(
      (data) => {
        ...
        console.log(
          'this.testCusMode.cusCarDetailsElmListDisplay[2]' +
            this.cusModel.cusCarDetailsElmListDisplay[2].plateId
        );
